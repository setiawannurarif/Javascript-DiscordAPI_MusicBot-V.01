...
module.exports = {
  ...
  callback: ({ message, client }) => {
    ...
    if (parseInt(command)) {
      ...
      return embedBuilder(client, message, `RED`, `Error: Volume ${command}`, `Use volume between 0-200`);
    }
    ...
    embedBuilder(client, message, `BLUE`, `Volume: `, `${message.author.tag} has changed the volume to: \`${command} %\`\n\n${status}`).then((msg) => {
