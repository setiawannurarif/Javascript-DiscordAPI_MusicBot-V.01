...
module.exports = {
  ...
  callback: ({ client, message }) => {
    ...
    if (0 <= Number(args[1]) && Number(args[1]) <= 2) {
      ...
      embedBuilder(client, message, `BLUE`, `Repeat mode set to: `, `${args[1].replace('0', 'OFF').replace('1', 'Repeat song').replace('2', 'Repeat Queue')}\n\n${status}`);
    } else {
      embedBuilder(client, message, `RED`, `Repeat mode error: `, `Please use a number: **\n1. \`0\`: set to OFF\n2. \`1\`: set to repeat a song\n3. \`2\`: set to repeat a queue**`).then((msg) => {
