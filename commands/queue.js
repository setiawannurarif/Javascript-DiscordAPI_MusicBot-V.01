...
module.exports = {
  ...
  callback: ({ client, message }) => {
    ...
    if (!queue) return;
    let curQueue = queue.songs.map((song, id) => `**${id + 1}**. [${song.name}](${song.url}) - \`${song.formattedDuration}\``).join('\n');

    embedBuilder(client, message, `BLUE`, `Queue: `, curQueue, message.author).then((msg) => {
