...
module.exports = {
  ...
  callback: async ({ client, message }) => {
    ...
    embedBuilder(client, message, "BLUE", `Searching ${command}`).then(
      (msg) => {
    ...
    let searchembed = await embedBuilder(
      ...
      "BLUE",
      "Result Search: ",
      `Choose number 1 between 15, and type the number correctly: \n${searchresult}`,
      message.author
    );
      ...
      .then((collected) => {
        ...
        if (isNaN(userinput)) {
          embedBuilder(
            ...
            "RED",
            "Not a right number!",
            "Choose between 0 - 15, Autoplay number 1!"
          ).then((msg) => {
    ...
    embedBuilder(
      ...
      "BLUE",
      "Playing a song!",
      `🎶 Started playing: [${result[userinput - 1].name}](${
